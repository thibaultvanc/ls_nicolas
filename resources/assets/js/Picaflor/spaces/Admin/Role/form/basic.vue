    ...
    <div class="well">
        <h2 v-show="mode=='update'">{{ $t('Picaflor.Role.edit') }}</h2>
        <h2 v-show="mode=='create'">{{ $t('Picaflor.Role.create') }}</h2>
        ...
        <form class="form-horizontal form-label-left" method="POST" action="/api/Picaflor/role" @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
    ...
    <label for="name" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Picaflor.Role.fields.name.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/5/tables/8/fields/33/edit" v-show="larastrapEditionMode">
          {{ $t('Picaflor.Role.fields.name.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="text" class="form-control" name="name" v-model="form.name" :placeholder="$t('Picaflor.Role.fields.name.placeholder')" >
    ...
    <label for="label" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Picaflor.Role.fields.label.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/5/tables/8/fields/34/edit" v-show="larastrapEditionMode">
          {{ $t('Picaflor.Role.fields.label.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="text" class="form-control" name="label" v-model="form.label" :placeholder="$t('Picaflor.Role.fields.label.placeholder')" >
...
label:'',
                ...
                methods: {
                        fetchObject () {
                            var vm=this
                            axios.get('/api/Picaflor/role/'+this.id).then(function (response) {
                    ...
                    submit(object) {
                        ...
                        if(this.mode == 'create'){
                            ...
                            this.form.submit('post','/api/Picaflor/role')
                              .then(response => {
                            ...
                            var vm=this
                              ...
                              this.form.submit('put','/api/Picaflor/role/'+this.form.id)
                                .then(response => {
                    ...
                    deleteObject(object) {
                        ...
                        var vm=this
                        axios.delete('/api/Picaflor/role/'+this.form.id, object).then(function (response) {
                        vm.sending=false
