      ...
      <br>
          ...
          <div class="tabbable">
              <ul class="nav nav-pills nav-stacked col-md-3">
                  ...
                  <li class=""><a href="#plans" data-toggle="tab">plans**HM**</a></li><li class=""><a href="#users" data-toggle="tab">users**BTM**</a></li><li class=""><a href="#permissions" data-toggle="tab">permissions**BTM**</a></li>
              </ul>
              <div class="tab-content col-md-8">
                  <div class="tab-pane fade in active" id="general">
                  <el-button v-if="mode=='show'" type="primary" @click="mode='form'">{{$t('core.edit')}}</el-button><el-button v-if="mode=='form'" @click="mode='show'">{{$t('core.cancel')}}</el-button><transition appear name="fade" mode="out-in"> <role-form v-if="mode=='form'" :id="getId"></role-form> </transition><transition appear name="fade" mode="in-out"> <role-show v-if="mode=='show'" :id="getId"></role-show> </transition>
                  </div>
                  
                                 <div class="tab-pane fade in" id="plans">
                                    <plans from="role" :role="id" :fromId="id" :url="getUrl('plans')"></plans>
                                 </div>

                                 <div class="tab-pane fade in" id="users">
                                    <users from="role" :role="id" :fromId="id" :url="getUrl('users')"></users>
                                 </div>

                                 <div class="tab-pane fade in" id="permissions">
                                    <permissions from="role" :role="id" :fromId="id" :url="getUrl('permissions')"></permissions>
                                 </div>
...
import permissions from '../../Permission/list/basic.vue'
        ...
        export default {
            ...
            methods: {
                goBack(){
                    window.location.replace('/Picaflor/role')
                },
                getUrl(methodName){

                    return '/api/Picaflor/role/'+this.getId+'/'+methodName
                },


                fetchObject () {
                    var vm=this
                    axios.get('/api/Picaflor/role/'+this.getId).then(function (response) {
