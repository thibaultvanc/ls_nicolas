    ...
    <div class="well">
        <h2 v-show="mode=='update'">{{ $t('Picaflor.RoleUser.edit') }}</h2>
        <h2 v-show="mode=='create'">{{ $t('Picaflor.RoleUser.create') }}</h2>
        ...
        <form class="form-horizontal form-label-left" method="POST" action="/api/Picaflor/role_user" @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
    ...
    <label for="user_id" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Picaflor.RoleUser.fields.user_id.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/5/tables/7/fields/30/edit" v-show="larastrapEditionMode">
          {{ $t('Picaflor.RoleUser.fields.user_id.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="number" class="form-control" name="user_id" v-model="form.user_id" :placeholder="$t('Picaflor.RoleUser.fields.user_id.placeholder')" >
    ...
    <label for="role_id" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Picaflor.RoleUser.fields.role_id.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/5/tables/7/fields/31/edit" v-show="larastrapEditionMode">
          {{ $t('Picaflor.RoleUser.fields.role_id.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="number" class="form-control" name="role_id" v-model="form.role_id" :placeholder="$t('Picaflor.RoleUser.fields.role_id.placeholder')" >
...
role_id:'',
                ...
                methods: {
                        fetchObject () {
                            var vm=this
                            axios.get('/api/Picaflor/role_user/'+this.id).then(function (response) {
                    ...
                    submit(object) {
                        ...
                        if(this.mode == 'create'){
                            ...
                            this.form.submit('post','/api/Picaflor/role_user')
                              .then(response => {
                            ...
                            var vm=this
                              ...
                              this.form.submit('put','/api/Picaflor/role_user/'+this.form.id)
                                .then(response => {
                    ...
                    deleteObject(object) {
                        ...
                        var vm=this
                        axios.delete('/api/Picaflor/role_user/'+this.form.id, object).then(function (response) {
                        vm.sending=false
