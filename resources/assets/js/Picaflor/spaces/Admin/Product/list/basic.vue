    ...
    <style>
            ...
            <div class="well">
                <h2>{{ $t('Picaflor.Product.index') }}</h2>
            </div>
            ...
            <a href="/admin/devis/5/tables/17/fields/create" class="btn ">{{ $t('larastrap.manageTable') }}</a>
            </div>
                ...
                <el-dropdown v-if="!selectListId.length">
                 ...
                 <div class="pull-right">
                    ...
                    <span ><el-button type="primary" @click="clickAdd" icon="plus" v-if="!selectListId.length">{{ $t('core.add') }}</el-button></span>
                                      ...
                                      <span v-if="!!fromId && from=='invoices'">
                                      <el-button type="primary" icon="plus" @click="clickAttach_products" v-if="!selectListId.length" >{{ $t('core.attach') }}  BTM</el-button>
                                      </span>
                                      <span v-if="!!fromId && from=='product_type'">
                             ...
                             <thead>
                                   <tr>
                                         <th>{{ $t('core.actions') }}</th>  <th>
                                                          ...
                                                          <a href="/admin/devis/5/tables/17/fields/56/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Product.fields.id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Name</span>
                                                          <a href="/admin/devis/5/tables/17/fields/57/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Product.fields.name.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Product_type_id</span>
                                                          <a href="/admin/devis/5/tables/17/fields/97/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Product.fields.product_type_id.label') }}
                                                          </a>
                                                        </th>
                                ...
                                <transition-group name="list" tag="tbody">
                                  <tr  v-for="object in filterdDataList" :key="object">
                                    <td>
                                        ...
                                        <el-button @click="clickDetach_Invoices(object)" icon="delete" size="mini"
                                                    v-show="!chooseMode && from && from=='invoices'" >{{ $t('core.detach') }} fromInvoices</el-button>
          ...
          {{ object.product_type.name }}
                    ...
                    <el-dialog title="Add new products" v-model="showmodal" size="large">
                        <product-form force-mode="create" :value="strangerValue"></product-form>
                      <span slot="footer" class="dialog-footer">
                        <el-button @click="showmodal = false">{{ $t('core.cancel') }}</el-button>
                            ...
                            <el-dialog title="Attach Existing products" v-model="showmodalAttach_products" size="large">

                            <product-list choose-mode :except-list.sync="dataList" :from="from" :from-id="fromId">
                              </product-list>

                              <span slot="footer" class="dialog-footer">
                                <el-button @click="closemodalAttach_products">{{ $t('core.cancel') }}</el-button>
                                <!--<el-button type="primary" @click="showmodalAttach_products = false, showmodal = true">{{ $t('core.createNew') }} products</el-button>-->
                              </span>
                            </el-dialog>
	...
	import productList from './basic.vue'
                          ...
                          fromId:{
                            ...
                            data(){
                                return {
                                    ...
                                    showmodalAttach_products:false
                            ...
                            methods: {
                                ...
                                // },
                                    ...
                                    closemodalAttach_products (){
                                      this.fetchData(products)
                                      this.showmodalAttach_devis = false

                                    },

                                    clickAttach_products (){
                                            // if(this.$route.params.id){
                                                this.showmodalAttach_products=!this.showmodalAttach_products
                                            // }else{
                                                // this.$router.push('/product/create')
                                            // }
                                    },
                                  ...
                                  submitAttachButton () {
                                    ...
                                    axios.post('/api/Picaflor/'+this.from+'/'+this.fromId+'/attach_products', this.selectListId).then(reponse=>{
                                      console.log('submitAttachButton')


                                      Event.$emit('attached', 'products')
                                ...
                                clickDelete (object){
                                  ...
                                  clickDetach_Invoices (item) {




                                    this.$confirm('This will remove the product in this '+this.from+'. Continue?', 'Warning', {
                                      confirmButtonText: this.$t('core.ok'),
                                      cancelButtonText: this.$t('core.cancel'),
                                      type: 'warning'
                                    }).then(() => {
                                      console.log('to detach', item)
                                      this.detach_Invoices (item)
                                  ...
                                  detach_Invoices  (item){
                                    // alert('detaching !')
                                    console.log('detaching !', item)
                                    var vm = this
                                    axios.post('/api/Picaflor/invoices/'+ this.fromId + '/detach_product/' + item.id).then(response=>{
                                ...
                                delete (object){
                                    var vm=this
                                    axios.delete('/api/Picaflor/product/'+ object.id).then(response=>{
                                ...
                                deleteMany (){
                                    var vm=this
                                    axios.post('/api/Picaflor/product/delete_many',this.selectListId).then(response=>{
                                ...
                                edit (object){
                                    window.location.replace('/Picaflor/Admin/product/'+object.id+'/edit');
                                },

                                show (object){
                                    window.location.replace('/Picaflor/Admin/product/'+object.id);
                                },
