    ...
    <div class="well">
        <h2 v-show="mode=='update'">{{ $t('Picaflor.OrganizationUser.edit') }}</h2>
        <h2 v-show="mode=='create'">{{ $t('Picaflor.OrganizationUser.create') }}</h2>
        ...
        <form class="form-horizontal form-label-left" method="POST" action="/api/Picaflor/organization_user" @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
    ...
    <label for="organization_id" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Picaflor.OrganizationUser.fields.organization_id.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/5/tables/31/fields/104/edit" v-show="larastrapEditionMode">
          {{ $t('Picaflor.OrganizationUser.fields.organization_id.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="number" class="form-control" name="organization_id" v-model="form.organization_id" :placeholder="$t('Picaflor.OrganizationUser.fields.organization_id.placeholder')" >
    ...
    <label for="user_id" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Picaflor.OrganizationUser.fields.user_id.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/5/tables/31/fields/105/edit" v-show="larastrapEditionMode">
          {{ $t('Picaflor.OrganizationUser.fields.user_id.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="number" class="form-control" name="user_id" v-model="form.user_id" :placeholder="$t('Picaflor.OrganizationUser.fields.user_id.placeholder')" >
            ...
            <div class="col-sm-12">
                ...
                <button @click.prevent="submit(object)" class="btn btn-primary pull-right" :disabled="form.errors.any()" >
                    ...
                    <b v-if="mode=='update'">{{$t('core.update')}}</b>
                     OrganizationUser <i class="fa fa-spin fa-spinner" v-show="sending"></i>
                 </button>
...
user_id:'',
                ...
                methods: {
                        fetchObject () {
                            var vm=this
                            axios.get('/api/Picaflor/organization_user/'+this.id).then(function (response) {
                    ...
                    submit(object) {
                        ...
                        if(this.mode == 'create'){
                            ...
                            this.form.submit('post','/api/Picaflor/organization_user')
                              .then(response => {
                                ...
                                this.$notify({
                                  ...
                                  Event.$emit('createdOrganizationUser', response.data)
                            ...
                            var vm=this
                              ...
                              this.form.submit('put','/api/Picaflor/organization_user/'+this.form.id)
                                .then(response => {
                                    ...
                                    // vm.$router.push('/organization_user')
                        });
                    ...
                    deleteObject(object) {
                        ...
                        var vm=this
                        axios.delete('/api/Picaflor/organization_user/'+this.form.id, object).then(function (response) {
                        vm.sending=false
