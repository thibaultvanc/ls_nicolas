    ...
    <style>
            ...
            <div class="well">
                <h2>{{ $t('Picaflor.Currency.index') }}</h2>
            </div>
            ...
            <a href="/admin/devis/5/tables/15/fields/create" class="btn ">{{ $t('larastrap.manageTable') }}</a>
            </div>
                             ...
                             <thead>
                                   <tr>
                                         <th>{{ $t('core.actions') }}</th>  <th>
                                                          ...
                                                          <a href="/admin/devis/5/tables/15/fields/52/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Currency.fields.id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Name</span>
                                                          <a href="/admin/devis/5/tables/15/fields/53/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Currency.fields.name.label') }}
                                                          </a>
                                                        </th>
                    ...
                    <el-dialog title="Add new currencies" v-model="showmodal" size="large">
                      ...
                      <span slot="footer" class="dialog-footer">
                        <el-button @click="showmodal = false">{{ $t('core.cancel') }}</el-button>
                            ...
                            <el-dialog title="Attach Existing currencies" v-model="showmodalAttach_currencies" size="large">

                            <currency-list choose-mode :except-list.sync="dataList" :from="from" :from-id="fromId">
                              </currency-list>

                              <span slot="footer" class="dialog-footer">
                                <el-button @click="closemodalAttach_currencies">{{ $t('core.cancel') }}</el-button>
                                <!--<el-button type="primary" @click="showmodalAttach_currencies = false, showmodal = true">{{ $t('core.createNew') }} currencies</el-button>-->
                              </span>
                            </el-dialog>
	...
	import currencyList from './basic.vue'
                          ...
                          fromId:{
                            ...
                            data(){
                                return {
                                    ...
                                    showmodalAttach_currencies:false
                            ...
                            methods: {
                                ...
                                // },
                                    ...
                                    closemodalAttach_currencies (){
                                      this.fetchData(currencies)
                                      this.showmodalAttach_devis = false

                                    },

                                    clickAttach_currencies (){
                                            // if(this.$route.params.id){
                                                this.showmodalAttach_currencies=!this.showmodalAttach_currencies
                                            // }else{
                                                // this.$router.push('/currency/create')
                                            // }
                                    },
                                  ...
                                  submitAttachButton () {
                                    ...
                                    axios.post('/api/Picaflor/'+this.from+'/'+this.fromId+'/attach_currencies', this.selectListId).then(reponse=>{
                                      console.log('submitAttachButton')


                                      Event.$emit('attached', 'currencies')
                                ...
                                delete (object){
                                    var vm=this
                                    axios.delete('/api/Picaflor/currency/'+ object.id).then(response=>{
                                ...
                                deleteMany (){
                                    var vm=this
                                    axios.post('/api/Picaflor/currency/delete_many',this.selectListId).then(response=>{
                                ...
                                edit (object){
                                    window.location.replace('/Picaflor/Admin/currency/'+object.id+'/edit');
                                },

                                show (object){
                                    window.location.replace('/Picaflor/Admin/currency/'+object.id);
                                },
