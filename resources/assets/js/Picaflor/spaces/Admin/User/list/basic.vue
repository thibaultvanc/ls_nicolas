    ...
    <style>
            ...
            <a href="/admin/devis/5/tables/6/fields/create" class="btn ">{{ $t('larastrap.manageTable') }}</a>
            </div>
                ...
                <el-dropdown v-if="!selectListId.length">
                 ...
                 <div class="pull-right">
                    ...
                    <span ><el-button type="primary" @click="clickAdd" icon="plus" v-if="!selectListId.length">{{ $t('core.add') }}</el-button></span>
                                      ...
                                      <el-button type="primary" icon="plus" @click="clickAttach_users" v-if="!selectListId.length" >{{ $t('core.attach') }}  BTM</el-button>
                                      </span>

                                      <span v-if="!!fromId && from=='firm'">
                                      <el-button type="primary" icon="plus" @click="clickAttach_users" v-if="!selectListId.length" >{{ $t('core.attach') }}  BTM</el-button>
                                      </span>

                                      <span v-if="!!fromId && from=='organization'">
                                      <el-button type="primary" icon="plus" @click="clickAttach_users" v-if="!selectListId.length" >{{ $t('core.attach') }}  BTM</el-button>
                                      </span>
 ...
 <th>
                                                          ...
                                                          {{ $t('Picaflor.User.fields.organization_id.label') }}
                                                          </a>
                                                        </th>
                                  ...
                                  <tr  v-for="object in filterdDataList" :key="object">
                                    <td>
                                        ...
                                        <el-button @click="clickDetach_Role(object)" icon="delete" size="mini"
                                                    v-show="!chooseMode && from && from=='role'" >{{ $t('core.detach') }} fromRole</el-button><el-button @click="clickDetach_Firm(object)" icon="delete" size="mini"
                                                    v-show="!chooseMode && from && from=='firm'" >{{ $t('core.detach') }} fromFirm</el-button><el-button @click="clickDetach_Organization(object)" icon="delete" size="mini"
                                                    v-show="!chooseMode && from && from=='organization'" >{{ $t('core.detach') }} fromOrganization</el-button>
                                ...
                                <td>		 {{object.id }} 
</a></td><td><a href="#" @click="edit(object)">		 {{object.firstname}} 
</a></td><td><a href="#" @click="edit(object)">		 {{object.lastname}} 
</a></td><td>		 {{object.phone}} 
</a></td><td>		 {{object.email}} 
</a></td><td>		 <i :class="{'el-icon-circle-check' : object.is_admin, 'el-icon-circle-cross' : !object.is_admin, }" ></i> 
                    ...
                    <el-dialog title="Add new users" v-model="showmodal" size="large">
                        <user-form force-mode="create" :value="strangerValue"></user-form>
                      <span slot="footer" class="dialog-footer">
                        <el-button @click="showmodal = false">{{ $t('core.cancel') }}</el-button>
                            ...
                            <el-dialog title="Attach Existing users" v-model="showmodalAttach_users" size="large">

                            <user-list choose-mode :except-list.sync="dataList" :from="from" :from-id="fromId">
                              </user-list>

                              <span slot="footer" class="dialog-footer">
                                <el-button @click="closemodalAttach_users">{{ $t('core.cancel') }}</el-button>
                                <!--<el-button type="primary" @click="showmodalAttach_users = false, showmodal = true">{{ $t('core.createNew') }} users</el-button>-->
                              </span>
                            </el-dialog>
	...
	import userList from './basic.vue'
...
organization:{
                            ...
                            methods: {
                                ...
                                // },
                                    ...
                                    clickAttach_users (){
                                            // if(this.$route.params.id){
                                                this.showmodalAttach_users=!this.showmodalAttach_users
                                            // }else{
                                                // this.$router.push('/user/create')
                                            // }
                                    },
                                  ...
                                  submitAttachButton () {
                                    ...
                                    axios.post('/api/Picaflor/'+this.from+'/'+this.fromId+'/attach_users', this.selectListId).then(reponse=>{
                                      console.log('submitAttachButton')


                                      Event.$emit('attached', 'users')
                                ...
                                clickDelete (object){
                                  ...
                                  clickDetach_Role (item) {
                                    ...
                                    this.$confirm('This will remove the user in this '+this.from+'. Continue?', 'Warning', {
                                      confirmButtonText: this.$t('core.ok'),
                                      cancelButtonText: this.$t('core.cancel'),
                                      type: 'warning'
                                    }).then(() => {
                                  ...
                                  detach_Role  (item){
                                    ...
                                    var vm = this
                                    axios.post('/api/Picaflor/role/'+ this.fromId + '/detach_user/' + item.id).then(response=>{
                                  ...
                                  detach_Firm  (item){
                                    ...
                                    console.log('detaching !', item)
                                    var vm = this
                                    axios.post('/api/Picaflor/firm/'+ this.fromId + '/detach_user/' + item.id).then(response=>{
                                      //Route::post("devis/{devis}/detach_product/1", "Arthes\Web\DevisApiController@attach_products");
                                      // var index = vm.dataList.indexOf(object)
                                      // vm.dataList.splice(index,1)
                                      // vm.refresh()
                                  ...
                                  detach_Organization  (item){
                                    // alert('detaching !')
                                    console.log('detaching !', item)
                                    var vm = this
                                    axios.post('/api/Picaflor/organization/'+ this.fromId + '/detach_user/' + item.id).then(response=>{
                                      //Route::post("devis/{devis}/detach_product/1", "Arthes\Web\DevisApiController@attach_products");
                                      // var index = vm.dataList.indexOf(object)
                                      // vm.dataList.splice(index,1)
                                      // vm.refresh()
                                ...
                                delete (object){
                                    var vm=this
                                    axios.delete('/api/Picaflor/user/'+ object.id).then(response=>{
                                ...
                                deleteMany (){
                                    var vm=this
                                    axios.post('/api/Picaflor/user/delete_many',this.selectListId).then(response=>{
                                ...
                                edit (object){
                                    window.location.replace('/Picaflor/Admin/user/'+object.id+'/edit');
                                },

                                show (object){
                                    window.location.replace('/Picaflor/Admin/user/'+object.id);
                                },
