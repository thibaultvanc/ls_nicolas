    ...
    <style>
      ...
      <div class="page-header">
        ...
        <h1>
        <small>
        <el-tag type="primary">firm_type</el-tag>
      ...
      <br>
          ...
          <div class="tabbable">
              <ul class="nav nav-pills nav-stacked col-md-3">
                  ...
                  <li class=""><a href="#firms" data-toggle="tab">firms**HM**</a></li>
              </ul>
              <div class="tab-content col-md-8">
                  <div class="tab-pane fade in active" id="general">
                  <el-button v-if="mode=='show'" type="primary" @click="mode='form'">{{$t('core.edit')}}</el-button><el-button v-if="mode=='form'" @click="mode='show'">{{$t('core.cancel')}}</el-button><transition appear name="fade" mode="out-in"> <firm-type-form v-if="mode=='form'" :id="getId"></firm-type-form> </transition><transition appear name="fade" mode="in-out"> <firm-type-show v-if="mode=='show'" :id="getId"></firm-type-show> </transition>
                  </div>
                  
                                 <div class="tab-pane fade in" id="firms">
                                    <firms from="firm_type" :firm_type="id" :fromId="id" :url="getUrl('firms')"></firms>
                                 </div>
...
import firms from '../..//list/basic.vue'
        ...
        export default {
            ...
            methods: {
                goBack(){
                    window.location.replace('/Picaflor/firm_type')
                },
                getUrl(methodName){

                    return '/api/Picaflor/firm_type/'+this.getId+'/'+methodName
                },


                fetchObject () {
                    var vm=this
                    axios.get('/api/Picaflor/firm_type/'+this.getId).then(function (response) {
