    ...
    <style>
      ...
      <div class="page-header">
        ...
        <h1>
        <small>
        <el-tag type="primary">payment_provider</el-tag>
      ...
      <br>
          ...
          <div class="tabbable">
              <ul class="nav nav-pills nav-stacked col-md-3">
                  ...
                  <li class=""><a href="#transactions" data-toggle="tab">transactions**HM**</a></li>
              </ul>
              <div class="tab-content col-md-8">
                  <div class="tab-pane fade in active" id="general">
                  <el-button v-if="mode=='show'" type="primary" @click="mode='form'">{{$t('core.edit')}}</el-button><el-button v-if="mode=='form'" @click="mode='show'">{{$t('core.cancel')}}</el-button><transition appear name="fade" mode="out-in"> <payment-provider-form v-if="mode=='form'" :id="getId"></payment-provider-form> </transition><transition appear name="fade" mode="in-out"> <payment-provider-show v-if="mode=='show'" :id="getId"></payment-provider-show> </transition>
                  </div>
                  
                                 <div class="tab-pane fade in" id="transactions">
                                    <transactions from="payment_provider" :payment_provider="id" :fromId="id" :url="getUrl('transactions')"></transactions>
                                 </div>
...
import transactions from '../../Transaction/list/basic.vue'
        ...
        export default {
            ...
            methods: {
                goBack(){
                    window.location.replace('/Picaflor/payment_provider')
                },
                getUrl(methodName){

                    return '/api/Picaflor/payment_provider/'+this.getId+'/'+methodName
                },


                fetchObject () {
                    var vm=this
                    axios.get('/api/Picaflor/payment_provider/'+this.getId).then(function (response) {
