    ...
    <div class="well">
        <h2 v-show="mode=='update'">{{ $t('Picaflor.PaymentProvider.edit') }}</h2>
        <h2 v-show="mode=='create'">{{ $t('Picaflor.PaymentProvider.create') }}</h2>
        ...
        <form class="form-horizontal form-label-left" method="POST" action="/api/Picaflor/payment_provider" @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
    ...
    <label for="name" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Picaflor.PaymentProvider.fields.name.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/5/tables/25/fields/78/edit" v-show="larastrapEditionMode">
          {{ $t('Picaflor.PaymentProvider.fields.name.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="text" class="form-control" name="name" v-model="form.name" :placeholder="$t('Picaflor.PaymentProvider.fields.name.placeholder')" >
            ...
            <div class="col-sm-12">
                ...
                <button @click.prevent="submit(object)" class="btn btn-primary pull-right" :disabled="form.errors.any()" >
                    ...
                    <b v-if="mode=='update'">{{$t('core.update')}}</b>
                     PaymentProvider <i class="fa fa-spin fa-spinner" v-show="sending"></i>
                 </button>
...
name:'',
                ...
                methods: {
                        fetchObject () {
                            var vm=this
                            axios.get('/api/Picaflor/payment_provider/'+this.id).then(function (response) {
                    ...
                    submit(object) {
                        ...
                        if(this.mode == 'create'){
                            ...
                            this.form.submit('post','/api/Picaflor/payment_provider')
                              .then(response => {
                                ...
                                this.$notify({
                                  ...
                                  Event.$emit('createdPaymentProvider', response.data)

                                
                                if (!this.forceMode) {
                                        
                                                this.$router.push('/payment_provider/'+this.form.id)
                                }
                            ...
                            var vm=this
                              ...
                              this.form.submit('put','/api/Picaflor/payment_provider/'+this.form.id)
                                .then(response => {
                                    ...
                                    // vm.$router.push('/payment_provider')
                        });
                    ...
                    deleteObject(object) {
                        ...
                        var vm=this
                        axios.delete('/api/Picaflor/payment_provider/'+this.form.id, object).then(function (response) {
                        vm.sending=false
