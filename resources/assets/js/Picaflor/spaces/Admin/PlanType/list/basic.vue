    ...
    <style>
            ...
            <div class="well">
                <h2>{{ $t('Picaflor.PlanType.index') }}</h2>
            </div>
            ...
            <a href="/admin/devis/5/tables/26/fields/create" class="btn ">{{ $t('larastrap.manageTable') }}</a>
            </div>
                             ...
                             <thead>
                                   <tr>
                                         <th>{{ $t('core.actions') }}</th>  <th>
                                                          ...
                                                          <a href="/admin/devis/5/tables/26/fields/84/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.PlanType.fields.id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Name</span>
                                                          <a href="/admin/devis/5/tables/26/fields/85/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.PlanType.fields.name.label') }}
                                                          </a>
                                                        </th>
                    ...
                    <el-dialog title="Add new plan_types" v-model="showmodal" size="large">
                        <plan-type-form force-mode="create" :value="strangerValue"></plan-type-form>
                      <span slot="footer" class="dialog-footer">
                        <el-button @click="showmodal = false">{{ $t('core.cancel') }}</el-button>
                            ...
                            <el-dialog title="Attach Existing plan_types" v-model="showmodalAttach_plan_types" size="large">

                            <plan-type-list choose-mode :except-list.sync="dataList" :from="from" :from-id="fromId">
                              </plan-type-list>

                              <span slot="footer" class="dialog-footer">
                                <el-button @click="closemodalAttach_plan_types">{{ $t('core.cancel') }}</el-button>
                                <!--<el-button type="primary" @click="showmodalAttach_plan_types = false, showmodal = true">{{ $t('core.createNew') }} plan_types</el-button>-->
                              </span>
                            </el-dialog>
                ...
                window.Event=new Vue()
                	...
                	import planTypeForm from '../form/basic.vue'
	import planTypeList from './basic.vue'
                          ...
                          fromId:{
                            ...
                            data(){
                                return {
                                    ...
                                    showmodalAttach_plan_types:false
                            ...
                            methods: {
                                ...
                                clickAdd (){
                                        ...
                                        // }else{
                                        //     this.$router.push('/plan_type/create')
                                        // }
                                },
                                ...
                                // },
                                    ...
                                    closemodalAttach_plan_types (){
                                      this.fetchData(plan_types)
                                      this.showmodalAttach_devis = false

                                    },

                                    clickAttach_plan_types (){
                                            // if(this.$route.params.id){
                                                this.showmodalAttach_plan_types=!this.showmodalAttach_plan_types
                                            // }else{
                                                // this.$router.push('/plan_type/create')
                                            // }
                                    },
                                  ...
                                  submitAttachButton () {
                                    ...
                                    axios.post('/api/Picaflor/'+this.from+'/'+this.fromId+'/attach_plan_types', this.selectListId).then(reponse=>{
                                      console.log('submitAttachButton')


                                      Event.$emit('attached', 'plan_types')
                                ...
                                delete (object){
                                    var vm=this
                                    axios.delete('/api/Picaflor/plan_type/'+ object.id).then(response=>{
                                ...
                                deleteMany (){
                                    var vm=this
                                    axios.post('/api/Picaflor/plan_type/delete_many',this.selectListId).then(response=>{
                                ...
                                edit (object){
                                    window.location.replace('/Picaflor/Admin/plan_type/'+object.id+'/edit');
                                },

                                show (object){
                                    window.location.replace('/Picaflor/Admin/plan_type/'+object.id);
                                },
