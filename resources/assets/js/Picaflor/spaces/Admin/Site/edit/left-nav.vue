    ...
    <style>
      ...
      <div class="page-header">
        ...
        <h1>
        <small>
        <el-tag type="primary">site</el-tag>
      ...
      <br>
          ...
          <div class="tabbable">
              <ul class="nav nav-pills nav-stacked col-md-3">
                  ...
                  <li class=""><a href="#invoices" data-toggle="tab">invoices**HM**</a></li><li class=""><a href="#plans" data-toggle="tab">plans**HM**</a></li>
              </ul>
              <div class="tab-content col-md-8">
                  <div class="tab-pane fade in active" id="general">
                  <el-button v-if="mode=='show'" type="primary" @click="mode='form'">{{$t('core.edit')}}</el-button><el-button v-if="mode=='form'" @click="mode='show'">{{$t('core.cancel')}}</el-button><transition appear name="fade" mode="out-in"> <site-form v-if="mode=='form'" :id="getId"></site-form> </transition><transition appear name="fade" mode="in-out"> <site-show v-if="mode=='show'" :id="getId"></site-show> </transition>
                  </div>
                  
                                 <div class="tab-pane fade in" id="invoices">
                                    <invoices from="site" :site="id" :fromId="id" :url="getUrl('invoices')"></invoices>
                                 </div>

                                 <div class="tab-pane fade in" id="plans">
                                    <plans from="site" :site="id" :fromId="id" :url="getUrl('plans')"></plans>
                                 </div>
...
import plans from '../../Plan/list/basic.vue'
        ...
        export default {
            ...
            methods: {
                goBack(){
                    window.location.replace('/Picaflor/site')
                },
                getUrl(methodName){

                    return '/api/Picaflor/site/'+this.getId+'/'+methodName
                },


                fetchObject () {
                    var vm=this
                    axios.get('/api/Picaflor/site/'+this.getId).then(function (response) {
