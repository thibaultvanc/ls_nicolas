    ...
    <style>
            ...
            <a href="/admin/devis/5/tables/21/fields/create" class="btn ">{{ $t('larastrap.manageTable') }}</a>
            </div>
                ...
                <el-dropdown v-if="!selectListId.length">
                 ...
                 <div class="pull-right">
                    ...
                    <span ><el-button type="primary" @click="clickAdd" icon="plus" v-if="!selectListId.length">{{ $t('core.add') }}</el-button></span>
                                      ...
                                      <span v-if="!!fromId && from=='plan_family'">
                                      <el-button type="primary" @click="clickAdd" icon="plus" >{{ $t('core.add') }}</el-button>
                                      </span>
                                      <span v-if="!!fromId && from=='currency'">
                                      <el-button type="primary" @click="clickAdd" icon="plus" >{{ $t('core.add') }}</el-button>
                                      </span>
                                      <span v-if="!!fromId && from=='plan_period'">
                                      <el-button type="primary" @click="clickAdd" icon="plus" >{{ $t('core.add') }}</el-button>
                                      </span>
                                      <span v-if="!!fromId && from=='role'">
                                      <el-button type="primary" @click="clickAdd" icon="plus" >{{ $t('core.add') }}</el-button>
                                      </span>
                                      <span v-if="!!fromId && from=='plan_type'">
                                      <el-button type="primary" @click="clickAdd" icon="plus" >{{ $t('core.add') }}</el-button>
                                      </span>
 ...
 <th>
                                                          ...
                                                          {{ $t('Picaflor.Plan.fields.currency_id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Plan_period_id</span>
                                                          <a href="/admin/devis/5/tables/21/fields/91/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Plan.fields.plan_period_id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Role_id</span>
                                                          <a href="/admin/devis/5/tables/21/fields/94/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Plan.fields.role_id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Plan_type_id</span>
                                                          <a href="/admin/devis/5/tables/21/fields/111/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Plan.fields.plan_type_id.label') }}
                                                          </a>
                                                        </th>
          ...
          {{ object.plan_type.name }}
                    ...
                    <el-dialog title="Add new plans" v-model="showmodal" size="large">
                        <plan-form force-mode="create" :value="strangerValue"></plan-form>
                      <span slot="footer" class="dialog-footer">
                        <el-button @click="showmodal = false">{{ $t('core.cancel') }}</el-button>
                            ...
                            <el-dialog title="Attach Existing plans" v-model="showmodalAttach_plans" size="large">

                            <plan-list choose-mode :except-list.sync="dataList" :from="from" :from-id="fromId">
                              </plan-list>

                              <span slot="footer" class="dialog-footer">
                                <el-button @click="closemodalAttach_plans">{{ $t('core.cancel') }}</el-button>
                                <!--<el-button type="primary" @click="showmodalAttach_plans = false, showmodal = true">{{ $t('core.createNew') }} plans</el-button>-->
                              </span>
                            </el-dialog>
                ...
                window.Event=new Vue()
                	...
                	import planForm from '../form/basic.vue'
	import planList from './basic.vue'
                          ...
                          fromId:{
                            ...
                            methods: {
                                ...
                                // },
                                    ...
                                    closemodalAttach_plans (){
                                      this.fetchData(plans)
                                    ...
                                    clickAttach_plans (){
                                            // if(this.$route.params.id){
                                                this.showmodalAttach_plans=!this.showmodalAttach_plans
                                            // }else{
                                                // this.$router.push('/plan/create')
                                            // }
                                    },
                                  ...
                                  submitAttachButton () {
                                    ...
                                    axios.post('/api/Picaflor/'+this.from+'/'+this.fromId+'/attach_plans', this.selectListId).then(reponse=>{
                                      console.log('submitAttachButton')


                                      Event.$emit('attached', 'plans')
                                ...
                                delete (object){
                                    var vm=this
                                    axios.delete('/api/Picaflor/plan/'+ object.id).then(response=>{
                                ...
                                deleteMany (){
                                    var vm=this
                                    axios.post('/api/Picaflor/plan/delete_many',this.selectListId).then(response=>{
                                ...
                                edit (object){
                                    window.location.replace('/Picaflor/Admin/plan/'+object.id+'/edit');
                                },

                                show (object){
                                    window.location.replace('/Picaflor/Admin/plan/'+object.id);
                                },
