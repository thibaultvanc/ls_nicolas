    ...
    <style>
      ...
      <div class="page-header">
        ...
        <h1>
        <small>
        <el-tag type="primary">plan</el-tag>
      ...
      <br>
          ...
          <div class="tabbable">
              <ul class="nav nav-pills nav-stacked col-md-3">
                  ...
                  <li class=""><a href="#subscriptions" data-toggle="tab">subscriptions**HM**</a></li>
              </ul>
              <div class="tab-content col-md-8">
                  <div class="tab-pane fade in active" id="general">
                  <el-button v-if="mode=='show'" type="primary" @click="mode='form'">{{$t('core.edit')}}</el-button><el-button v-if="mode=='form'" @click="mode='show'">{{$t('core.cancel')}}</el-button><transition appear name="fade" mode="out-in"> <plan-form v-if="mode=='form'" :id="getId"></plan-form> </transition><transition appear name="fade" mode="in-out"> <plan-show v-if="mode=='show'" :id="getId"></plan-show> </transition>
                  </div>
                  
                                 <div class="tab-pane fade in" id="subscriptions">
                                    <subscriptions from="plan" :plan="id" :fromId="id" :url="getUrl('subscriptions')"></subscriptions>
                                 </div>
...
import subscriptions from '../../Subscription/list/basic.vue'
        ...
        export default {
            ...
            methods: {
                goBack(){
                    window.location.replace('/Picaflor/plan')
                },
                getUrl(methodName){

                    return '/api/Picaflor/plan/'+this.getId+'/'+methodName
                },
