    ...
    <style>
            ...
            <a href="/admin/devis/5/tables/11/fields/create" class="btn ">{{ $t('larastrap.manageTable') }}</a>
            </div>
                ...
                <el-dropdown v-if="!selectListId.length">
                 ...
                 <div class="pull-right">
                    ...
                    <span ><el-button type="primary" @click="clickAdd" icon="plus" v-if="!selectListId.length">{{ $t('core.add') }}</el-button></span>
                                      ...
                                      <span v-if="!!fromId && from=='user'">
                                      <el-button type="primary" icon="plus" @click="clickAttach_firms" v-if="!selectListId.length" >{{ $t('core.attach') }}  BTM</el-button>
                                      </span>
                                      ...
                                      <span v-if="!!fromId && from=='country'">
                                      <el-button type="primary" @click="clickAdd" icon="plus" >{{ $t('core.add') }}</el-button>
                                      </span>
                                      <span v-if="!!fromId && from=='firm_type'">
                                      <el-button type="primary" @click="clickAdd" icon="plus" >{{ $t('core.add') }}</el-button>
                                      </span>
                             ...
                             <thead>
                                   <tr>
                                         <th>{{ $t('core.actions') }}</th>  <th>
                                                          ...
                                                          <a href="/admin/devis/5/tables/11/fields/41/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Firm.fields.id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Nom de la marque</span>
                                                          <a href="/admin/devis/5/tables/11/fields/42/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Firm.fields.name.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Pays d'origine de la marque</span>
                                                          <a href="/admin/devis/5/tables/11/fields/50/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Firm.fields.country_id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Firm_type_id</span>
                                                          <a href="/admin/devis/5/tables/11/fields/132/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Firm.fields.firm_type_id.label') }}
                                                          </a>
                                                        </th>
                                ...
                                <transition-group name="list" tag="tbody">
                                  <tr  v-for="object in filterdDataList" :key="object">
                                    <td>
                                        ...
                                        <el-button @click="clickDetach_User(object)" icon="delete" size="mini"
                                                    v-show="!chooseMode && from && from=='user'" >{{ $t('core.detach') }} fromUser</el-button><el-button @click="clickDetach_Firm(object)" icon="delete" size="mini"
                                                    v-show="!chooseMode && from && from=='firm'" >{{ $t('core.detach') }} fromFirm</el-button><el-button @click="clickDetach_Firm(object)" icon="delete" size="mini"
                                                    v-show="!chooseMode && from && from=='firm'" >{{ $t('core.detach') }} fromFirm</el-button>
          ...
          {{ object.firm_type.name }}
                    ...
                    <el-dialog title="Add new firms" v-model="showmodal" size="large">
                        <firm-form force-mode="create" :value="strangerValue"></firm-form>
                      <span slot="footer" class="dialog-footer">
                        <el-button @click="showmodal = false">{{ $t('core.cancel') }}</el-button>
                            ...
                            <el-dialog title="Attach Existing firms" v-model="showmodalAttach_firms" size="large">

                            <firm-list choose-mode :except-list.sync="dataList" :from="from" :from-id="fromId">
                              </firm-list>

                              <span slot="footer" class="dialog-footer">
                                <el-button @click="closemodalAttach_firms">{{ $t('core.cancel') }}</el-button>
                                <!--<el-button type="primary" @click="showmodalAttach_firms = false, showmodal = true">{{ $t('core.createNew') }} firms</el-button>-->
                              </span>
                            </el-dialog>
                ...
                window.Event=new Vue()
                	...
                	import firmForm from '../form/basic.vue'
	import firmList from './basic.vue'
...
firm:{
                            ...
                            methods: {
                                ...
                                // },
                                    ...
                                    closemodalAttach_firms (){
                                      this.fetchData(firms)
                                    ...
                                    clickAttach_firms (){
                                            // if(this.$route.params.id){
                                                this.showmodalAttach_firms=!this.showmodalAttach_firms
                                            // }else{
                                                // this.$router.push('/firm/create')
                                            // }
                                    },
                                  ...
                                  submitAttachButton () {
                                    ...
                                    axios.post('/api/Picaflor/'+this.from+'/'+this.fromId+'/attach_firms', this.selectListId).then(reponse=>{
                                      console.log('submitAttachButton')


                                      Event.$emit('attached', 'firms')
                                ...
                                clickDelete (object){
                                  ...
                                  clickDetach_User (item) {




                                    this.$confirm('This will remove the firm in this '+this.from+'. Continue?', 'Warning', {
                                      confirmButtonText: this.$t('core.ok'),
                                      cancelButtonText: this.$t('core.cancel'),
                                      type: 'warning'
                                    }).then(() => {
                                      console.log('to detach', item)
                                      this.detach_User (item)
                                  ...
                                  detach_User  (item){
                                    ...
                                    var vm = this
                                    axios.post('/api/Picaflor/user/'+ this.fromId + '/detach_firm/' + item.id).then(response=>{
                                  ...
                                  detach_Firm  (item){
                                    ...
                                    console.log('detaching !', item)
                                    var vm = this
                                    axios.post('/api/Picaflor/firm/'+ this.fromId + '/detach_firm/' + item.id).then(response=>{
                                      //Route::post("devis/{devis}/detach_product/1", "Arthes\Web\DevisApiController@attach_products");
                                      // var index = vm.dataList.indexOf(object)
                                      // vm.dataList.splice(index,1)
                                      // vm.refresh()
                                ...
                                delete (object){
                                    var vm=this
                                    axios.delete('/api/Picaflor/firm/'+ object.id).then(response=>{
                                ...
                                deleteMany (){
                                    var vm=this
                                    axios.post('/api/Picaflor/firm/delete_many',this.selectListId).then(response=>{
                                ...
                                edit (object){
                                    window.location.replace('/Picaflor/Admin/firm/'+object.id+'/edit');
                                },

                                show (object){
                                    window.location.replace('/Picaflor/Admin/firm/'+object.id);
                                },
