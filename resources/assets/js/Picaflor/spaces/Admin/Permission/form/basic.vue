    ...
    <div class="well">
        <h2 v-show="mode=='update'">{{ $t('Picaflor.Permission.edit') }}</h2>
        <h2 v-show="mode=='create'">{{ $t('Picaflor.Permission.create') }}</h2>
        ...
        <form class="form-horizontal form-label-left" method="POST" action="/api/Picaflor/permission" @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
    ...
    <label for="name" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Picaflor.Permission.fields.name.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/5/tables/9/fields/36/edit" v-show="larastrapEditionMode">
          {{ $t('Picaflor.Permission.fields.name.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="text" class="form-control" name="name" v-model="form.name" :placeholder="$t('Picaflor.Permission.fields.name.placeholder')" >
    ...
    <label for="label" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Picaflor.Permission.fields.label.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/5/tables/9/fields/37/edit" v-show="larastrapEditionMode">
          {{ $t('Picaflor.Permission.fields.label.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="text" class="form-control" name="label" v-model="form.label" :placeholder="$t('Picaflor.Permission.fields.label.placeholder')" >
...
label:'',
                ...
                methods: {
                        fetchObject () {
                            var vm=this
                            axios.get('/api/Picaflor/permission/'+this.id).then(function (response) {
                    ...
                    submit(object) {
                        ...
                        if(this.mode == 'create'){
                            ...
                            this.form.submit('post','/api/Picaflor/permission')
                              .then(response => {
                            ...
                            var vm=this
                              ...
                              this.form.submit('put','/api/Picaflor/permission/'+this.form.id)
                                .then(response => {
                    ...
                    deleteObject(object) {
                        ...
                        var vm=this
                        axios.delete('/api/Picaflor/permission/'+this.form.id, object).then(function (response) {
                        vm.sending=false
