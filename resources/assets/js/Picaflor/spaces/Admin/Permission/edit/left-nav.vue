...
import roles from '../../Role/list/basic.vue'
        ...
        export default {
            ...
            methods: {
                goBack(){
                    window.location.replace('/Picaflor/permission')
                },
                getUrl(methodName){

                    return '/api/Picaflor/permission/'+this.getId+'/'+methodName
                },


                fetchObject () {
                    var vm=this
                    axios.get('/api/Picaflor/permission/'+this.getId).then(function (response) {
