    ...
    <style>
            ...
            <div class="well">
                <h2>{{ $t('Picaflor.Invoices.index') }}</h2>
            </div>
            ...
            <a href="/admin/devis/5/tables/16/fields/create" class="btn ">{{ $t('larastrap.manageTable') }}</a>
            </div>
                ...
                <el-dropdown v-if="!selectListId.length">
                 ...
                 <div class="pull-right">
                    ...
                    <span ><el-button type="primary" @click="clickAdd" icon="plus" v-if="!selectListId.length">{{ $t('core.add') }}</el-button></span>
                                      ...
                                      <span v-if="!!fromId && from=='product'">
                                      <el-button type="primary" icon="plus" @click="clickAttach_invoices" v-if="!selectListId.length" >{{ $t('core.attach') }}  BTM</el-button>
                                      </span>
                                      <span v-if="!!fromId && from=='site'">
                                      <el-button type="primary" @click="clickAdd" icon="plus" >{{ $t('core.add') }}</el-button>
                                      </span>
                                      <span v-if="!!fromId && from=='user'">
                                      <el-button type="primary" @click="clickAdd" icon="plus" >{{ $t('core.add') }}</el-button>
                                      </span>
                                      <span v-if="!!fromId && from=='currency'">
                                      <el-button type="primary" @click="clickAdd" icon="plus" >{{ $t('core.add') }}</el-button>
                                      </span>
                             ...
                             <thead>
                                   <tr>
                                         <th>{{ $t('core.actions') }}</th>  <th>
                                                          ...
                                                          <a href="/admin/devis/5/tables/16/fields/54/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Invoices.fields.id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Name</span>
                                                          <a href="/admin/devis/5/tables/16/fields/55/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Invoices.fields.name.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Site_id</span>
                                                          <a href="/admin/devis/5/tables/16/fields/67/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Invoices.fields.site_id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">User_id</span>
                                                          <a href="/admin/devis/5/tables/16/fields/81/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.Invoices.fields.user_id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          ...
                                                          {{ $t('Picaflor.Invoices.fields.organization_id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          ...
                                                          {{ $t('Picaflor.Invoices.fields.currency_id.label') }}
                                                          </a>
                                                        </th>
                                ...
                                <transition-group name="list" tag="tbody">
                                  <tr  v-for="object in filterdDataList" :key="object">
                                    <td>
                                        ...
                                        <el-button @click="clickDetach_Product(object)" icon="delete" size="mini"
                                                    v-show="!chooseMode && from && from=='product'" >{{ $t('core.detach') }} fromProduct</el-button>
          ...
          {{ object.currency.name }}
                    ...
                    <el-dialog title="Add new invoices" v-model="showmodal" size="large">
                        <invoices-form force-mode="create" :value="strangerValue"></invoices-form>
                      <span slot="footer" class="dialog-footer">
                        <el-button @click="showmodal = false">{{ $t('core.cancel') }}</el-button>
                            ...
                            <el-dialog title="Attach Existing invoices" v-model="showmodalAttach_invoices" size="large">

                            <invoices-list choose-mode :except-list.sync="dataList" :from="from" :from-id="fromId">
                              </invoices-list>

                              <span slot="footer" class="dialog-footer">
                                <el-button @click="closemodalAttach_invoices">{{ $t('core.cancel') }}</el-button>
                                <!--<el-button type="primary" @click="showmodalAttach_invoices = false, showmodal = true">{{ $t('core.createNew') }} invoices</el-button>-->
                              </span>
                            </el-dialog>
	...
	import invoicesList from './basic.vue'
                          ...
                          fromId:{
                            ...
                            data(){
                                return {
                                    ...
                                    showmodalAttach_invoices:false
                            ...
                            methods: {
                                ...
                                // },
                                    ...
                                    closemodalAttach_invoices (){
                                      this.fetchData(invoices)
                                      this.showmodalAttach_devis = false

                                    },

                                    clickAttach_invoices (){
                                            // if(this.$route.params.id){
                                                this.showmodalAttach_invoices=!this.showmodalAttach_invoices
                                            // }else{
                                                // this.$router.push('/invoices/create')
                                            // }
                                    },
                                  ...
                                  submitAttachButton () {
                                    ...
                                    axios.post('/api/Picaflor/'+this.from+'/'+this.fromId+'/attach_invoices', this.selectListId).then(reponse=>{
                                      console.log('submitAttachButton')


                                      Event.$emit('attached', 'invoices')
                                ...
                                clickDelete (object){
                                  ...
                                  clickDetach_Product (item) {




                                    this.$confirm('This will remove the invoices in this '+this.from+'. Continue?', 'Warning', {
                                      confirmButtonText: this.$t('core.ok'),
                                      cancelButtonText: this.$t('core.cancel'),
                                      type: 'warning'
                                    }).then(() => {
                                      console.log('to detach', item)
                                      this.detach_Product (item)
                                  ...
                                  detach_Product  (item){
                                    ...
                                    var vm = this
                                    axios.post('/api/Picaflor/product/'+ this.fromId + '/detach_invoices/' + item.id).then(response=>{
                                ...
                                delete (object){
                                    var vm=this
                                    axios.delete('/api/Picaflor/invoices/'+ object.id).then(response=>{
                                ...
                                deleteMany (){
                                    var vm=this
                                    axios.post('/api/Picaflor/invoices/delete_many',this.selectListId).then(response=>{
                                ...
                                edit (object){
                                    window.location.replace('/Picaflor/Admin/invoices/'+object.id+'/edit');
                                },

                                show (object){
                                    window.location.replace('/Picaflor/Admin/invoices/'+object.id);
                                },
