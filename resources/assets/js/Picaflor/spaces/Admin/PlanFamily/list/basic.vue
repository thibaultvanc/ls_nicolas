    ...
    <style>
            ...
            <div class="well">
                <h2>{{ $t('Picaflor.PlanFamily.index') }}</h2>
            </div>
            ...
            <!-- <h1>PlanFamily  List</h1>-->
            <!--<pre>chooseMode : {{chooseMode}}</pre>
            <pre>exceptList : {{exceptList}}</pre>-->

            <div v-show="larastrapEditionMode">
            <a href="/admin/devis/5/tables/23/fields/create" class="btn ">{{ $t('larastrap.manageTable') }}</a>
            </div>
                             ...
                             <thead>
                                   <tr>
                                         <th>{{ $t('core.actions') }}</th>  <th>
                                                          ...
                                                          <a href="/admin/devis/5/tables/23/fields/72/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.PlanFamily.fields.id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Name</span>
                                                          <a href="/admin/devis/5/tables/23/fields/73/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.PlanFamily.fields.name.label') }}
                                                          </a>
                                                        </th>
                    ...
                    <el-dialog title="Add new plan_families" v-model="showmodal" size="large">
                        <plan-family-form force-mode="create" :value="strangerValue"></plan-family-form>
                      <span slot="footer" class="dialog-footer">
                        <el-button @click="showmodal = false">{{ $t('core.cancel') }}</el-button>
                            ...
                            <el-dialog title="Attach Existing plan_families" v-model="showmodalAttach_plan_families" size="large">

                            <plan-family-list choose-mode :except-list.sync="dataList" :from="from" :from-id="fromId">
                              </plan-family-list>

                              <span slot="footer" class="dialog-footer">
                                <el-button @click="closemodalAttach_plan_families">{{ $t('core.cancel') }}</el-button>
                                <!--<el-button type="primary" @click="showmodalAttach_plan_families = false, showmodal = true">{{ $t('core.createNew') }} plan_families</el-button>-->
                              </span>
                            </el-dialog>
                ...
                window.Event=new Vue()
                	...
                	import planFamilyForm from '../form/basic.vue'
	import planFamilyList from './basic.vue'


                    export default {
                        name: 'plan-family-list',
                        components: {
                                				planFamilyForm, 
				planFamilyList, 
                        ...
                        props :{
                          ...
                          fromId:{
                            ...
                            data(){
                                return {
                                    ...
                                    showmodalAttach_plan_families:false
                            ...
                            methods: {
                                ...
                                clickAdd (){
                                        ...
                                        // }else{
                                        //     this.$router.push('/plan_family/create')
                                        // }
                                },
                                ...
                                // },
                                    ...
                                    closemodalAttach_plan_families (){
                                      this.fetchData(plan_families)
                                      this.showmodalAttach_devis = false

                                    },

                                    clickAttach_plan_families (){
                                            // if(this.$route.params.id){
                                                this.showmodalAttach_plan_families=!this.showmodalAttach_plan_families
                                            // }else{
                                                // this.$router.push('/plan_family/create')
                                            // }
                                    },
                                  ...
                                  submitAttachButton () {
                                    ...
                                    axios.post('/api/Picaflor/'+this.from+'/'+this.fromId+'/attach_plan_families', this.selectListId).then(reponse=>{
                                      console.log('submitAttachButton')


                                      Event.$emit('attached', 'plan_families')
                                ...
                                delete (object){
                                    var vm=this
                                    axios.delete('/api/Picaflor/plan_family/'+ object.id).then(response=>{
                                ...
                                deleteMany (){
                                    var vm=this
                                    axios.post('/api/Picaflor/plan_family/delete_many',this.selectListId).then(response=>{
                                ...
                                edit (object){
                                    window.location.replace('/Picaflor/Admin/plan_family/'+object.id+'/edit');
                                },

                                show (object){
                                    window.location.replace('/Picaflor/Admin/plan_family/'+object.id);
                                },
                          ...
                          computed:{
                              ...
                              fetchUrl (){
                                  ...
                                  return '/api/Picaflor/plan_family'
                              },
