    ...
    <div class="well">
        <h2 v-show="mode=='update'">{{ $t('Picaflor.ProductType.edit') }}</h2>
        <h2 v-show="mode=='create'">{{ $t('Picaflor.ProductType.create') }}</h2>
        ...
        <form class="form-horizontal form-label-left" method="POST" action="/api/Picaflor/product_type" @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
    ...
    <label for="name" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Picaflor.ProductType.fields.name.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/5/tables/29/fields/96/edit" v-show="larastrapEditionMode">
          {{ $t('Picaflor.ProductType.fields.name.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="text" class="form-control" name="name" v-model="form.name" :placeholder="$t('Picaflor.ProductType.fields.name.placeholder')" >
            ...
            <div class="col-sm-12">
                ...
                <button @click.prevent="submit(object)" class="btn btn-primary pull-right" :disabled="form.errors.any()" >
                    ...
                    <b v-if="mode=='update'">{{$t('core.update')}}</b>
                     ProductType <i class="fa fa-spin fa-spinner" v-show="sending"></i>
                 </button>
...
name:'',
                ...
                methods: {
                        fetchObject () {
                            var vm=this
                            axios.get('/api/Picaflor/product_type/'+this.id).then(function (response) {
                    ...
                    submit(object) {
                        ...
                        if(this.mode == 'create'){
                            ...
                            this.form.submit('post','/api/Picaflor/product_type')
                              .then(response => {
                                ...
                                this.$notify({
                                  ...
                                  Event.$emit('createdProductType', response.data)

                                
                                if (!this.forceMode) {
                                        
                                                this.$router.push('/product_type/'+this.form.id)
                                }
                            ...
                            var vm=this
                              ...
                              this.form.submit('put','/api/Picaflor/product_type/'+this.form.id)
                                .then(response => {
                    ...
                    deleteObject(object) {
                        ...
                        var vm=this
                        axios.delete('/api/Picaflor/product_type/'+this.form.id, object).then(function (response) {
                        vm.sending=false
