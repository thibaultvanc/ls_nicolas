    ...
    <style>
      ...
      <div class="page-header">
        ...
        <h1>
        <small>
        <el-tag type="primary">product_type</el-tag>
      ...
      <br>
          ...
          <div class="tabbable">
              <ul class="nav nav-pills nav-stacked col-md-3">
                  ...
                  <li class=""><a href="#products" data-toggle="tab">products**HM**</a></li>
              </ul>
              <div class="tab-content col-md-8">
                  <div class="tab-pane fade in active" id="general">
                  <el-button v-if="mode=='show'" type="primary" @click="mode='form'">{{$t('core.edit')}}</el-button><el-button v-if="mode=='form'" @click="mode='show'">{{$t('core.cancel')}}</el-button><transition appear name="fade" mode="out-in"> <product-type-form v-if="mode=='form'" :id="getId"></product-type-form> </transition><transition appear name="fade" mode="in-out"> <product-type-show v-if="mode=='show'" :id="getId"></product-type-show> </transition>
                  </div>
                  
                                 <div class="tab-pane fade in" id="products">
                                    <products from="product_type" :product_type="id" :fromId="id" :url="getUrl('products')"></products>
                                 </div>
...
import products from '../..//list/basic.vue'
        ...
        export default {
            ...
            methods: {
                goBack(){
                    window.location.replace('/Picaflor/product_type')
                },
                getUrl(methodName){

                    return '/api/Picaflor/product_type/'+this.getId+'/'+methodName
                },


                fetchObject () {
                    var vm=this
                    axios.get('/api/Picaflor/product_type/'+this.getId).then(function (response) {
