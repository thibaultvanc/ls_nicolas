    ...
    <style>
            ...
            <div class="well">
                <h2>{{ $t('Picaflor.PlanPeriod.index') }}</h2>
            </div>
            ...
            <!-- <h1>PlanPeriod  List</h1>-->
            <!--<pre>chooseMode : {{chooseMode}}</pre>
            <pre>exceptList : {{exceptList}}</pre>-->

            <div v-show="larastrapEditionMode">
            <a href="/admin/devis/5/tables/24/fields/create" class="btn ">{{ $t('larastrap.manageTable') }}</a>
            </div>
                             ...
                             <thead>
                                   <tr>
                                         <th>{{ $t('core.actions') }}</th>  <th>
                                                          ...
                                                          <a href="/admin/devis/5/tables/24/fields/74/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.PlanPeriod.fields.id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Name</span>
                                                          <a href="/admin/devis/5/tables/24/fields/75/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Picaflor.PlanPeriod.fields.name.label') }}
                                                          </a>
                                                        </th>
                    ...
                    <el-dialog title="Add new plan_periods" v-model="showmodal" size="large">
                        <plan-period-form force-mode="create" :value="strangerValue"></plan-period-form>
                      <span slot="footer" class="dialog-footer">
                        <el-button @click="showmodal = false">{{ $t('core.cancel') }}</el-button>
                            ...
                            <el-dialog title="Attach Existing plan_periods" v-model="showmodalAttach_plan_periods" size="large">

                            <plan-period-list choose-mode :except-list.sync="dataList" :from="from" :from-id="fromId">
                              </plan-period-list>

                              <span slot="footer" class="dialog-footer">
                                <el-button @click="closemodalAttach_plan_periods">{{ $t('core.cancel') }}</el-button>
                                <!--<el-button type="primary" @click="showmodalAttach_plan_periods = false, showmodal = true">{{ $t('core.createNew') }} plan_periods</el-button>-->
                              </span>
                            </el-dialog>
	...
	import planPeriodList from './basic.vue'


                    export default {
                        name: 'plan-period-list',
                        components: {
                                				planPeriodForm, 
				planPeriodList, 
                        ...
                        props :{
                          ...
                          fromId:{
                            ...
                            data(){
                                return {
                                    ...
                                    showmodalAttach_plan_periods:false
                            ...
                            methods: {
                                ...
                                clickAdd (){
                                        ...
                                        // }else{
                                        //     this.$router.push('/plan_period/create')
                                        // }
                                },
                                ...
                                // },
                                    ...
                                    closemodalAttach_plan_periods (){
                                      this.fetchData(plan_periods)
                                      this.showmodalAttach_devis = false

                                    },

                                    clickAttach_plan_periods (){
                                            // if(this.$route.params.id){
                                                this.showmodalAttach_plan_periods=!this.showmodalAttach_plan_periods
                                            // }else{
                                                // this.$router.push('/plan_period/create')
                                            // }
                                    },
                                  ...
                                  submitAttachButton () {
                                    ...
                                    axios.post('/api/Picaflor/'+this.from+'/'+this.fromId+'/attach_plan_periods', this.selectListId).then(reponse=>{
                                      console.log('submitAttachButton')


                                      Event.$emit('attached', 'plan_periods')
                                ...
                                delete (object){
                                    var vm=this
                                    axios.delete('/api/Picaflor/plan_period/'+ object.id).then(response=>{
                                ...
                                deleteMany (){
                                    var vm=this
                                    axios.post('/api/Picaflor/plan_period/delete_many',this.selectListId).then(response=>{
                                ...
                                edit (object){
                                    window.location.replace('/Picaflor/Admin/plan_period/'+object.id+'/edit');
                                },

                                show (object){
                                    window.location.replace('/Picaflor/Admin/plan_period/'+object.id);
                                },
                          ...
                          computed:{
                              ...
                              fetchUrl (){
                                  ...
                                  return '/api/Picaflor/plan_period'
                              },
