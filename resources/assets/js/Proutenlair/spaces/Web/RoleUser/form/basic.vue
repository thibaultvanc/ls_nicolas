    ...
    <div class="well">
        <h2 v-show="mode=='update'">{{ $t('Proutenlair.RoleUser.edit') }}</h2>
        <h2 v-show="mode=='create'">{{ $t('Proutenlair.RoleUser.create') }}</h2>
        ...
        <form class="form-horizontal form-label-left" method="POST" action="/api/Proutenlair/role_user" @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
    ...
    <label for="user_id" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Proutenlair.RoleUser.fields.user_id.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/18/tables/64/fields/249/edit" v-show="larastrapEditionMode">
          {{ $t('Proutenlair.RoleUser.fields.user_id.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="number" class="form-control" name="user_id" v-model="form.user_id" :placeholder="$t('Proutenlair.RoleUser.fields.user_id.placeholder')" >
    ...
    <label for="role_id" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Proutenlair.RoleUser.fields.role_id.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/18/tables/64/fields/250/edit" v-show="larastrapEditionMode">
          {{ $t('Proutenlair.RoleUser.fields.role_id.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="number" class="form-control" name="role_id" v-model="form.role_id" :placeholder="$t('Proutenlair.RoleUser.fields.role_id.placeholder')" >
            ...
            <div class="col-sm-12">
                ...
                <button @click.prevent="submit(object)" class="btn btn-primary pull-right" :disabled="form.errors.any()" >
                    ...
                    <b v-if="mode=='update'">{{$t('core.update')}}</b>
                     RoleUser <i class="fa fa-spin fa-spinner" v-show="sending"></i>
                 </button>
...
role_id:'',
                ...
                methods: {
                        fetchObject () {
                            var vm=this
                            axios.get('/api/Proutenlair/role_user/'+this.id).then(function (response) {
                    ...
                    submit(object) {
                        ...
                        if(this.mode == 'create'){
                            ...
                            this.form.submit('post','/api/Proutenlair/role_user')
                              .then(response => {
                                ...
                                this.$notify({
                                  ...
                                  Event.$emit('createdRoleUser', response.data)
                            ...
                            var vm=this
                              ...
                              this.form.submit('put','/api/Proutenlair/role_user/'+this.form.id)
                                .then(response => {
                    ...
                    deleteObject(object) {
                        ...
                        var vm=this
                        axios.delete('/api/Proutenlair/role_user/'+this.form.id, object).then(function (response) {
                        vm.sending=false
