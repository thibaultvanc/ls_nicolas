    ...
    <style>
      ...
      <div class="page-header">
        ...
        <h1>
        <small>
        <el-tag type="primary">permission</el-tag>
      ...
      <br>
          ...
          <div class="tabbable">
              ...
              <div class="tab-content col-md-8">
                  ...
                  <el-button v-if="mode=='show'" type="primary" @click="mode='form'">{{$t('core.edit')}}</el-button><el-button v-if="mode=='form'" @click="mode='show'">{{$t('core.cancel')}}</el-button><transition appear name="fade" mode="out-in"> <permission-form v-if="mode=='form'" :id="getId"></permission-form> </transition><transition appear name="fade" mode="in-out"> <permission-show v-if="mode=='show'" :id="getId"></permission-show> </transition>
                  </div>
                  
                                 <div class="tab-pane fade in" id="roles">
                                    <roles from="permission" :permission="id" :fromId="id" :url="getUrl('roles')"></roles>
                                 </div>
...
import roles from '../../Role/list/basic.vue'
        ...
        export default {
            ...
            methods: {
                goBack(){
                    window.location.replace('/Proutenlair/permission')
                },
                getUrl(methodName){

                    return '/api/Proutenlair/permission/'+this.getId+'/'+methodName
                },


                fetchObject () {
                    var vm=this
                    axios.get('/api/Proutenlair/permission/'+this.getId).then(function (response) {
