    ...
    <div class="well">
        <h2 v-show="mode=='update'">{{ $t('Proutenlair.Permission.edit') }}</h2>
        <h2 v-show="mode=='create'">{{ $t('Proutenlair.Permission.create') }}</h2>
        ...
        <form class="form-horizontal form-label-left" method="POST" action="/api/Proutenlair/permission" @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
    ...
    <label for="label" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Proutenlair.Permission.fields.label.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/18/tables/66/fields/256/edit" v-show="larastrapEditionMode">
          {{ $t('Proutenlair.Permission.fields.label.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="text" class="form-control" name="label" v-model="form.label" :placeholder="$t('Proutenlair.Permission.fields.label.placeholder')" >

          <div class="errors">
          <span class="help is-danger" v-if="form.errors.has('label')" v-text="form.errors.get('label')"></span>
          </div>
    ...
    <label for="name" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Proutenlair.Permission.fields.name.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/18/tables/66/fields/255/edit" v-show="larastrapEditionMode">
          {{ $t('Proutenlair.Permission.fields.name.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="text" class="form-control" name="name" v-model="form.name" :placeholder="$t('Proutenlair.Permission.fields.name.placeholder')" >
...
label:'',
                ...
                methods: {
                        fetchObject () {
                            var vm=this
                            axios.get('/api/Proutenlair/permission/'+this.id).then(function (response) {
                    ...
                    submit(object) {
                        ...
                        if(this.mode == 'create'){
                            ...
                            this.form.submit('post','/api/Proutenlair/permission')
                              .then(response => {
                                ...
                                this.$notify({
                                  ...
                                  Event.$emit('createdPermission', response.data)

                                
                                if (!this.forceMode) {
                                        
                                                this.$router.push('/permission/'+this.form.id)
                                }
                            ...
                            var vm=this
                              ...
                              this.form.submit('put','/api/Proutenlair/permission/'+this.form.id)
                                .then(response => {
                    ...
                    deleteObject(object) {
                        ...
                        var vm=this
                        axios.delete('/api/Proutenlair/permission/'+this.form.id, object).then(function (response) {
                        vm.sending=false
