    ...
    <style>
            ...
            <div class="well">
                <h2>{{ $t('Proutenlair.Role.index') }}</h2>
            </div>
            ...
            <a href="/admin/devis/18/tables/65/fields/create" class="btn ">{{ $t('larastrap.manageTable') }}</a>
            </div>
                ...
                <el-dropdown v-if="!selectListId.length">
                 ...
                 <div class="pull-right">
                    ...
                    <span ><el-button type="primary" @click="clickAdd" icon="plus" v-if="!selectListId.length">{{ $t('core.add') }}</el-button></span>
                                      ...
                                      <el-button type="primary" icon="plus" @click="clickAttach_roles" v-if="!selectListId.length" >{{ $t('core.attach') }}  BTM</el-button>
                                      </span>

                                      <span v-if="!!fromId && from=='permission'">
                                      <el-button type="primary" icon="plus" @click="clickAttach_roles" v-if="!selectListId.length" >{{ $t('core.attach') }}  BTM</el-button>
                                      </span>
                             ...
                             <thead>
                                   <tr>
                                         <th>{{ $t('core.actions') }}</th>  <th>
                                                          ...
                                                          <a href="/admin/devis/18/tables/65/fields/251/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Proutenlair.Role.fields.id.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Name</span>
                                                          <a href="/admin/devis/18/tables/65/fields/252/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Proutenlair.Role.fields.name.label') }}
                                                          </a>
                                                        </th>
 <th>
                                                          <span v-show="!larastrapEditionMode">Label</span>
                                                          <a href="/admin/devis/18/tables/65/fields/253/edit" v-show="larastrapEditionMode">
                                                          {{ $t('Proutenlair.Role.fields.label.label') }}
                                                          </a>
                                                        </th>
                                  ...
                                  <tr  v-for="object in filterdDataList" :key="object">
                                    <td>
                                        ...
                                        <el-button @click="clickDetach_User(object)" icon="delete" size="mini"
                                                    v-show="!chooseMode && from && from=='user'" >{{ $t('core.detach') }} fromUser</el-button><el-button @click="clickDetach_Permission(object)" icon="delete" size="mini"
                                                    v-show="!chooseMode && from && from=='permission'" >{{ $t('core.detach') }} fromPermission</el-button>
                    ...
                    <el-dialog title="Add new roles" v-model="showmodal" size="large">
                        <role-form force-mode="create" :value="strangerValue"></role-form>
                      <span slot="footer" class="dialog-footer">
                        <el-button @click="showmodal = false">{{ $t('core.cancel') }}</el-button>
                            ...
                            <el-dialog title="Attach Existing roles" v-model="showmodalAttach_roles" size="large">

                            <role-list choose-mode :except-list.sync="dataList" :from="from" :from-id="fromId">
                              </role-list>

                              <span slot="footer" class="dialog-footer">
                                <el-button @click="closemodalAttach_roles">{{ $t('core.cancel') }}</el-button>
                                <!--<el-button type="primary" @click="showmodalAttach_roles = false, showmodal = true">{{ $t('core.createNew') }} roles</el-button>-->
                              </span>
                            </el-dialog>
                ...
                window.Event=new Vue()
                	...
                	import roleForm from '../form/basic.vue'
	import roleList from './basic.vue'
...
permission:{
                            ...
                            methods: {
                                ...
                                // },
                                    ...
                                    closemodalAttach_roles (){
                                      this.fetchData(roles)
                                    ...
                                    clickAttach_roles (){
                                            // if(this.$route.params.id){
                                                this.showmodalAttach_roles=!this.showmodalAttach_roles
                                            // }else{
                                                // this.$router.push('/role/create')
                                            // }
                                    },
                                  ...
                                  submitAttachButton () {
                                    ...
                                    axios.post('/api/Proutenlair/'+this.from+'/'+this.fromId+'/attach_roles', this.selectListId).then(reponse=>{
                                      console.log('submitAttachButton')


                                      Event.$emit('attached', 'roles')
                                ...
                                clickDelete (object){
                                  ...
                                  clickDetach_User (item) {
                                    ...
                                    this.$confirm('This will remove the role in this '+this.from+'. Continue?', 'Warning', {
                                      confirmButtonText: this.$t('core.ok'),
                                      cancelButtonText: this.$t('core.cancel'),
                                      type: 'warning'
                                    }).then(() => {
                                  ...
                                  detach_User  (item){
                                    ...
                                    var vm = this
                                    axios.post('/api/Proutenlair/user/'+ this.fromId + '/detach_role/' + item.id).then(response=>{
                                  ...
                                  detach_Permission  (item){
                                    // alert('detaching !')
                                    console.log('detaching !', item)
                                    var vm = this
                                    axios.post('/api/Proutenlair/permission/'+ this.fromId + '/detach_role/' + item.id).then(response=>{
                                      //Route::post("devis/{devis}/detach_product/1", "Arthes\Web\DevisApiController@attach_products");
                                      // var index = vm.dataList.indexOf(object)
                                      // vm.dataList.splice(index,1)
                                      // vm.refresh()
                                ...
                                delete (object){
                                    var vm=this
                                    axios.delete('/api/Proutenlair/role/'+ object.id).then(response=>{
                                ...
                                deleteMany (){
                                    var vm=this
                                    axios.post('/api/Proutenlair/role/delete_many',this.selectListId).then(response=>{
                                ...
                                edit (object){
                                    window.location.replace('/Proutenlair/role/'+object.id+'/edit');
                                },

                                show (object){
                                    window.location.replace('/Proutenlair/role/'+object.id);
                                },
