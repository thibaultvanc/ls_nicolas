    ...
    <div class="well">
        <h2 v-show="mode=='update'">{{ $t('Proutenlair.Role.edit') }}</h2>
        <h2 v-show="mode=='create'">{{ $t('Proutenlair.Role.create') }}</h2>
        ...
        <form class="form-horizontal form-label-left" method="POST" action="/api/Proutenlair/role" @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
    ...
    <label for="name" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Proutenlair.Role.fields.name.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/18/tables/65/fields/252/edit" v-show="larastrapEditionMode">
          {{ $t('Proutenlair.Role.fields.name.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="text" class="form-control" name="name" v-model="form.name" :placeholder="$t('Proutenlair.Role.fields.name.placeholder')" >
    ...
    <label for="label" class="col-sm-2 control-label">
      <span v-show="!larastrapEditionMode">{{ $t('Proutenlair.Role.fields.label.label') }} <span class="text-danger">*</span></span>
      <a href="/admin/devis/18/tables/65/fields/253/edit" v-show="larastrapEditionMode">
          {{ $t('Proutenlair.Role.fields.label.label') }} <span class="text-danger">*</span>
      </a>

    </label>
    <div class="col-sm-10 "> 
		
        <input type="text" class="form-control" name="label" v-model="form.label" :placeholder="$t('Proutenlair.Role.fields.label.placeholder')" >
...
label:'',
                ...
                methods: {
                        fetchObject () {
                            var vm=this
                            axios.get('/api/Proutenlair/role/'+this.id).then(function (response) {
                    ...
                    submit(object) {
                        ...
                        if(this.mode == 'create'){
                            ...
                            this.form.submit('post','/api/Proutenlair/role')
                              .then(response => {
                                ...
                                this.$notify({
                                  ...
                                  Event.$emit('createdRole', response.data)

                                
                                if (!this.forceMode) {
                                        
                                                this.$router.push('/role/'+this.form.id)
                                }
                            ...
                            var vm=this
                              ...
                              this.form.submit('put','/api/Proutenlair/role/'+this.form.id)
                                .then(response => {
                    ...
                    deleteObject(object) {
                        ...
                        var vm=this
                        axios.delete('/api/Proutenlair/role/'+this.form.id, object).then(function (response) {
                        vm.sending=false
