...
<item-form v-if="mode=='form'" :id="getId"></item-form>
</transition>
<transition appear name="slide-fade">
<item-show v-if="mode=='show'" :id="getId"></item-show>
</transition>
</section>
...
<script>
	...
	import itemForm from '../Item/form.vue'
        ...
        export default {
            ...
            methods: {
                goBack(){
                    window.location.replace('/Arthes/item')
                },
                getUrl(methodName){

                    return '/api/Arthes/item/'+this.getId+'/'+methodName
                },


                fetchObject () {
                    var vm=this
                    axios.get('/api/Arthes/item/'+this.getId).then(function (response) {
